...
const SubmissionHistory = ({ problemId }) => {
  ...
  const formatMemory = (memory) => (memory < 1024 ? `${memory} kB` : `${(memory / 1024).toFixed(2)} MB`);
          ...
          <tbody>
            {submissions.map((sub, index) => (
              <tr key={sub._id}>
                ...
                <td>
                  <button className="btn btn-s btn-outline" onClick={() => setSelectedSubmission(sub)}>
                    Code
                  </button>
                </td>
              </tr>
          ...
          <div className="modal-box w-11/12 max-w-5xl">
            ...
            <div className="mb-4">
              <div className="flex flex-wrap gap-2 mb-2">
                ...
                <span className="badge badge-outline">Runtime: {selectedSubmission.runtime}s</span>
                <span className="badge badge-outline">Memory: {formatMemory(selectedSubmission.memory)}</span>
              ...
              {selectedSubmission.errorMessage && (
                <div className="alert alert-error mt-2">
                  <div>{selectedSubmission.errorMessage}</div>
                </div>
              )}
            </div>
